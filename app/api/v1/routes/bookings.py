...
async def _maybe_auto_assign(booking_id_str: str, booking_number: str, triggered_by_user_id: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # 1b. Check dispatch window (8 AM – 11 PM IST)
            if not await _is_dispatch_window(db):
                from datetime import datetime, timezone, timedelta
                _IST = timezone(timedelta(hours=5, minutes=30))
                _hour = datetime.now(_IST).hour
                _logger.info(
                    f"Auto-assign OUTSIDE dispatch window ({_hour:02d}:xx IST) "
                    f"— queuing booking {booking_number} for 8 AM sweep"
                )
                # Queue the booking so 8 AM sweep picks it up
                _booking_early = (await db.execute(
                    select(Booking).where(Booking.id == _UUID(booking_id_str))
                )).scalar_one_or_none()
                if _booking_early and not _booking_early.technician_id and                    _booking_early.status in (BookingStatus.PENDING, BookingStatus.CONFIRMED):
                    existing_notes = _booking_early.notes or ""
                    if "[PENDING_AUTO_ASSIGN]" not in existing_notes:
                        _booking_early.notes = (existing_notes + "\n[PENDING_AUTO_ASSIGN]").strip()
                    await db.commit()
                return
...
async def _sweep_pending_auto_assign(triggered_by_user_id: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Check dispatch window — don't sweep outside 8 AM–11 PM IST
            if not await _is_dispatch_window(db):
                from datetime import datetime, timezone, timedelta
                _IST = timezone(timedelta(hours=5, minutes=30))
                _logger.info(
                    f"Sweep skipped — outside dispatch window "
                    f"({datetime.now(_IST).hour:02d}:xx IST)"
                )
                return
...
@router.get("/today-summary", summary="Admin ops summary strip for today [Admin/CCO]")
async def today_summary(
    current_user: dict = Depends(AdminOrCCO),
    db: AsyncSession = Depends(get_db),
):
    """Returns a quick ops summary for the bookings page header strip."""
    from sqlalchemy import cast as _cast, Date as _SADate, func as _func, case as _sa_case
    from datetime import datetime, timezone, timedelta
    _IST = timezone(timedelta(hours=5, minutes=30))
    ...
    ACTIVE_STATUSES = [
        ...
        BookingStatus.ACCEPTED, BookingStatus.TECHNICIAN_ACCEPTED,
        BookingStatus.EN_ROUTE, BookingStatus.ARRIVED, BookingStatus.INSPECTING,
        BookingStatus.IN_PROGRESS, BookingStatus.PENDING_VERIFICATION,
        BookingStatus.INVOICE_GENERATED, BookingStatus.PAYMENT_PENDING,
    ]

    today_q = select(Booking).where(
        _cast(Booking.scheduled_date, _SADate) == _today
    )
    today_bookings = (await db.execute(today_q)).scalars().all()

    total_today       = len(today_bookings)
    urgent_count      = sum(1 for b in today_bookings if b.priority == "URGENT")
    unassigned_count  = sum(1 for b in today_bookings if not b.technician_id and b.status in ACTIVE_STATUSES)
    cancel_pending    = sum(1 for b in today_bookings if b.status == BookingStatus.CANCELLATION_REQUESTED)
    overdue_count     = (await db.execute(
        select(_func.count(Booking.id)).where(
            _cast(Booking.scheduled_date, _SADate) < _today,
...
async def list_bookings(
    ...
    sort_by: str = Query("smart", description="smart|created_at|scheduled_date|priority|status"),
    sort_order: str = Query("asc", description="asc|desc"),
    current_user: dict = Depends(AnyAuthenticated),
    db: AsyncSession = Depends(get_db)
):
    from app.models.service import Service
    from sqlalchemy import or_, and_, cast, String as SAString, case as sa_case, Date as SADate
    ...
    # ── Smart sort: urgency rank + scheduled date + priority, overridable ──
    STATUS_URGENCY = sa_case(
        (Booking.status == BookingStatus.CANCELLATION_REQUESTED, 1),
        (Booking.status == BookingStatus.PENDING,      2),
        (Booking.status == BookingStatus.CONFIRMED,    3),
        (Booking.status == BookingStatus.ASSIGNED,     4),
        (Booking.status == BookingStatus.ACCEPTED,     5),
        (Booking.status == BookingStatus.TECHNICIAN_ACCEPTED, 6),
        (Booking.status == BookingStatus.EN_ROUTE,     7),
        (Booking.status == BookingStatus.ARRIVED,      8),
        (Booking.status == BookingStatus.INSPECTING,   9),
        (Booking.status == BookingStatus.IN_PROGRESS,  10),
        (Booking.status == BookingStatus.PENDING_VERIFICATION, 11),
        (Booking.status == BookingStatus.COMPLETED,    12),
        (Booking.status == BookingStatus.INVOICE_GENERATED, 13),
        (Booking.status == BookingStatus.PAYMENT_PENDING, 14),
        else_=99,
    )
    PRIORITY_URGENCY = sa_case(
        (Booking.priority == "URGENT", 1),
        (Booking.priority == "HIGH",   2),
        else_=3,
    )
    _desc = sort_order.lower() == "desc"
    if sort_by == "scheduled_date":
        _ord = Booking.scheduled_date.desc() if _desc else Booking.scheduled_date.asc().nulls_last()
    elif sort_by == "created_at":
        _ord = Booking.created_at.desc() if _desc else Booking.created_at.asc()
    elif sort_by == "priority":
        _ord = PRIORITY_URGENCY.desc() if _desc else PRIORITY_URGENCY.asc()
    elif sort_by == "status":
        _ord = STATUS_URGENCY.desc() if _desc else STATUS_URGENCY.asc()
    else:  # "smart" default
        _ord = None  # handled below as multi-column

    if _ord is not None:
        _q_final = q.order_by(_ord, Booking.created_at.desc())
    else:
        _q_final = q.order_by(
            cast(Booking.scheduled_date, SADate).asc().nulls_last(),
            STATUS_URGENCY.asc(),
    ...
    rows = (await db.execute(
        _q_final.offset((page-1)*per_page).limit(per_page)
    )).all()
    ...
    for idx, (b, cust, svc, tech, domain, city_row) in enumerate(rows):
        ...
        items.append({
            ...
            # is_overdue: scheduled date is in the past and booking is still active
            "is_overdue": (
                b.scheduled_date is not None
                and b.scheduled_date.date() < __import__('datetime').date.today()
                and b.status.value not in (
                    "COMPLETED", "CANCELLED", "CLOSED", "PAID", "SETTLED",
...
# ── Invoice + Payment enrichment helper (used by GET /{booking_id}) ──────────
async def _enrich_booking_invoice_payment(db: AsyncSession, booking_id) -> dict:
    """Returns invoice and payment fields for the booking detail response."""
    ...
    try:
        ...
        latest_inv = invoices[0] if invoices else None
        total_paid = sum(
            p.amount for p in payments
            if p.status.value in ("SUCCESS", "COMPLETED", "VERIFIED")
        )
        total_pending = sum(
            p.amount for p in payments
            if p.status.value in ("PENDING",)
        )

        return {
            "invoice_number": latest_inv.invoice_number if latest_inv else None,
            "invoice_status": latest_inv.status.value if latest_inv else None,
            "invoice_total": float(latest_inv.total_amount) if latest_inv else None,
        ...
        import logging as _log
        _log.getLogger(__name__).warning(f"_enrich_booking_invoice_payment failed: {_err}")
        return {
            "invoice_number": None, "invoice_status": None,
            "invoice_total": None, "invoice_count": 0,
            "payments": [], "total_paid": 0.0, "total_pending": 0.0,
        }
...
async def get_booking(booking_id: UUID, current_user: dict = Depends(AnyAuthenticated), db: AsyncSession = Depends(get_db)):
    ...
    return success_response(data={
        ...
        # ── Invoice + payment enrichment ──────────────────────────────────
        **await _enrich_booking_invoice_payment(db, b.id),
    })
