...
async def _enrich_wallets(db, wallets):
    ...
    for w in wallets:
        ...
        result.append({
            ...
            "balance":         int(round(w.balance or 0)),
            "total_earned":    int(round(w.total_earned or 0)),
            "total_withdrawn": int(round(w.total_withdrawn or 0)),
...
async def list_wallets(
    ...
    return success_response(data={
        ...
        "summary": {
            "total_balance":    int(round(float(agg.total_balance))),
            "total_earned":     int(round(float(agg.total_earned))),
            "total_withdrawn":  int(round(float(agg.total_withdrawn))),
...
async def admin_list_settlements(
    ...
    for b in bookings:
        ...
        rows.append({
            ...
            "technician_code":  tech.technician_code if tech else None,
            "commission_total": int(round(total_commission)),
