...
async def _auto_retry_unassigned_bookings():
    """
    Background task: runs ONCE DAILY at 08:00 IST (same time as morning sweep).
    ...
    while True:
        try:
            # ── Wait until 08:00 IST today (or tomorrow if already past) ──────
            # IST = UTC+5:30
            now_utc = datetime.now(timezone.utc)
            now_ist = now_utc + timedelta(hours=5, minutes=30)
            # Target: 08:00 IST today
            target_ist = now_ist.replace(hour=8, minute=1, second=0, microsecond=0)
            ...
            _logger.info(f"[AUTO-RETRY] Next run at 08:00 IST — sleeping {sleep_seconds/3600:.1f} h")
...
-- ── REFERRALS: tables added in later migrations (safe to create if missing) ─
...
async def _dispatch_morning_sweep():
    """
    Background task: fires EXACTLY at 08:00 AM IST every day.

    Purpose: When a booking was created outside the dispatch window (11 PM – 8 AM),
    it is flagged [PENDING_AUTO_ASSIGN]. This task wakes at 8 AM and assigns all
    such queued bookings to the best available online technician.

    This is separate from _auto_retry_unassigned_bookings (which also runs at 8 AM
    and handles historical unassigned bookings). This task is specifically for the
    overnight queue created outside dispatch hours.
    """
    import asyncio
    import logging
    from datetime import datetime, timezone, timedelta
    ...
    while True:
        try:
            # ── Calculate seconds until next 08:00 AM IST ──────────────────
            IST = timezone(timedelta(hours=5, minutes=30))
            now_ist = datetime.now(IST)
            target = now_ist.replace(hour=8, minute=0, second=30, microsecond=0)
            if now_ist >= target:
                target = target + timedelta(days=1)
            sleep_secs = (target - now_ist).total_seconds()
            _logger.info(f"[MORNING-SWEEP] Next dispatch window open sweep in {sleep_secs/3600:.1f} h (08:00 IST)")
            await asyncio.sleep(sleep_secs)

            _logger.info("[MORNING-SWEEP] 08:00 AM IST — running dispatch window open sweep")

            # Get a system user ID for assigned_by FK
            from app.core.database import AsyncSessionLocal
            from app.models.user import User
            from sqlalchemy import select
            async with AsyncSessionLocal() as _db:
                _admin = (await _db.execute(
                    select(User).where(
                        User.role.in_(["SUPER_ADMIN", "ADMIN"]),
                        User.is_active == True,
                    ).limit(1)
                )).scalar_one_or_none()
                _uid = str(_admin.id) if _admin else None

            if _uid:
                from app.api.v1.routes.bookings import _sweep_pending_auto_assign
                await _sweep_pending_auto_assign(_uid)
                _logger.info("[MORNING-SWEEP] Morning sweep complete")
            else:
                _logger.warning("[MORNING-SWEEP] No admin user found — sweep skipped")

        except asyncio.CancelledError:
            raise
        except Exception as e:
            _logger.warning(f"[MORNING-SWEEP] Error: {e}", exc_info=True)
            await asyncio.sleep(60)
...
async def lifespan(app: FastAPI):
    ...
    morning_sweep_task       = asyncio.ensure_future(_dispatch_morning_sweep())

    yield
    # ── shutdown ─────────────────────────────────────────────
    await stop_redis_subscriber()

    for task in (auto_offline_task, auto_retry_task, pay_later_reminder_task, morning_sweep_task):
        if not task.done():
            task.cancel()
            try:
                await task
